...
const fromLines = (text: string) =>
  text
    ...
    .map((v) => v.trim())
    .filter((v) => v.length > 0);
const toNumberLines = (values?: number[]) => (values ?? []).join('\n');
const fromNumberLines = (text: string) =>
  text
    .split(/\r?\n/g)
    .map((v) => v.trim())
    .filter((v) => v.length > 0)
...
export function HandlerEditorBody({ open, mode, handlerName, onCancel, onSaved, onDeleted }: EditorProps) {
  ...
  const updateHttp = (patch: Partial<HttpConfigDraft>) => {
    setDraft((d) => {
      const current = (d.http ?? {}) as HttpConfigDraft;
      return { ...d, http: { ...current, ...patch } };
    });
  };

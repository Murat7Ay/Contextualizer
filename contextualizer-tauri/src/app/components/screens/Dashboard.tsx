...
export function Dashboard() {
  ...
  useEffect(() => {
    if (!handlersLoaded) {
      handlersList().then((list) => {
        useHandlersStore.getState().setHandlers(
          list.map((h) => ({
            name: h.name,
            type: h.handlerType,
            enabled: h.enabled,
    ...
    if (!exchangeLoaded && !exchangeLoading) {
      ...
      exchangeList().then((pkgs) => {
        useHandlerExchangeStore.getState().setPackages(
          pkgs.map((p) => ({
            id: p.handlerId,
            name: p.name,
  ...
  }, [handlersLoaded, cronLoaded, exchangeLoaded, exchangeLoading, setExchangeLoading]);
  ...
  const canUseHost = true;

  const runSelectedManual = () => {
    if (!selectedManual) return;
    manualHandlerExecute(selectedManual).catch(console.error);
  };
      ...
      <Card className="mb-8">
        ...
        <CardContent className="flex flex-col md:flex-row md:items-end gap-3">
          ...
          <div className="flex gap-2">
            <Button onClick={runSelectedManual} disabled={!selectedManual}>
              Run
            </Button>
            <Button
              variant="outline"
              onClick={() => handlerReload().catch(console.error)}
